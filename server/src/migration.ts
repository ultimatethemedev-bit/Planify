...
export async function runMigration(): Promise<void> {
  const oldIndexes: Record<string, string> = {
  ...
  for (const [collName, indexName] of Object.entries(oldIndexes)) {
    try {
      ...
      logger.info(`Dropped old index ${indexName} on ${collName}`)
    } catch {
  ...
  try {
    ...
    logger.info('Dropped old index userId_1 on settings')
  } catch {
    // Index doesn't exist
  }
  ...
  for (const user of users) {
    for (let i = 0; i < user.stores.length; i++) {
      ...
      if (!existingStore) {
        ...
        await store.save()
        logger.info(`Created Store: ${embeddedStore.name}`)
      }
      ...
      if (!settingsWithStoreId) {
        ...
        if (oldSettings && i === 0) {
          ...
          logger.info(`Updated settings for store: ${embeddedStore.name}`)
        } else {
          ...
          logger.info(`Created default settings for store: ${embeddedStore.name}`)
        }
      }
    }
  }

  logger.info(`Migration completed: ${users.length} user(s) migrated.`)
}
